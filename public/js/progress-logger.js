...
const ProgressLogger = {
  ...
  // Track last logged phase per hash to avoid flooding duplicate phase entries
  _lastLoggedPhase: {},
      ...
      return `
        <div class="hash-result-item ${statusClass}">
          ...
          <div class="hash-result-details">
            <div class="hash-result-detail"><span>Type:</span><strong>${result.passwordType || '\u2014'}</strong></div>
            <div class="hash-result-detail"><span>Attempts:</span><strong>${this.formatNumber(result.attempts || 0)}</strong></div>
            <div class="hash-result-detail"><span>Duration:</span><strong>${this.formatDuration(result.duration || 0)}</strong></div>
            <div class="hash-result-detail"><span>Speed:</span><strong>${this.formatSpeed(result.avgSpeed || 0)}</strong></div>
          </div>
        </div>
      `;
    }).join('');
  ...
  // Update performance metrics — called on the UI update interval, not per worker tick.
  // speed should be the already-aggregated total across all workers.
  updatePerformance(speed) {
    this.sessionStats.speedSamples.push(speed);
    // Keep rolling window of last 20 samples
    if (this.sessionStats.speedSamples.length > 20) this.sessionStats.speedSamples.shift();
    // Simple arithmetic mean — good enough, avoids noisy instantaneous spikes
  ...
  onHashStart(hashId, hash, passwordType) {
    this.updateHashResult(hashId, { hash, passwordType, status: 'processing', startTime: Date.now() });
  ...
  onHashCracked(hashId, password, attempts, duration) {
    ...
    this.updateHashResult(hashId, { status: 'cracked', password, attempts, duration, avgSpeed });
    ...
    // Clear phase tracking for this hash
    delete this._lastLoggedPhase[hashId];
  },
  ...
  onHashFailed(hashId, attempts, duration) {
    ...
    this.sessionStats.totalAttempts += attempts;
    const avgSpeed = attempts / (duration / 1000);
    this.updateHashResult(hashId, { status: 'failed', attempts, duration, avgSpeed });
    this.logEvent('error', `\u274C Failed to crack hash after ${this.formatNumber(attempts)} attempts`);
    delete this._lastLoggedPhase[hashId];
  },

  // Progress update — called on the UI update interval.
  // speed here is already the aggregated total across all active threads.
  onProgress(progress) {
    if (progress.speed) {
  ...
  // Phase change — only logs once per (hashId, phase) pair to prevent flooding.
  // threadCount is optional: shown in the log message when switching to brute-force.
  onPhaseChange(phase, hashId, threadCount) {
    const key = hashId || '__global__';
    if (this._lastLoggedPhase[key] === phase) return; // already logged this phase — skip
    this._lastLoggedPhase[key] = phase;

    let phaseText;
    if (phase === 'dictionary') {
      phaseText = '\uD83D\uDCDA Dictionary attack';
    } else {
      const t = threadCount ? ` (${threadCount} thread${threadCount !== 1 ? 's' : ''})` : '';
      phaseText = `\uD83D\uDD22 Brute-force started${t}`;
    }
